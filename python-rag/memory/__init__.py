...
class ConversationMemory:
    ...
    CATEGORIES = [
        ...
        "运动鞋", "拖鞋", "帽子", "墨镜",
        "面膜", "眼霜", "卸妆", "洁面", "爽肤水", "乳液", "眉笔", "化妆水", "蜜粉", "散粉", "唇釉", "精华液", "粉底液", "卸妆油",
        "护肤品", "化妆品", "美妆", "护肤", "彩妆",
        "充电器", "充电宝", "数据线", "键盘", "鼠标",
    ...
    CATEGORY_ALIASES = {
        # ── 美妆护肤 → 化妆品（子品类 + 别名 + 大类同义词） ──
        "防晒": "化妆品", "防晒霜": "化妆品",
        "洗面": "化妆品", "洗面奶": "化妆品", "洁面": "化妆品",
        "面霜": "化妆品",
        "精华": "化妆品", "精华液": "化妆品",
        "粉底": "化妆品", "粉底液": "化妆品",
        "口红": "化妆品", "唇釉": "化妆品",
        ...
        "蜜粉": "化妆品", "散粉": "化妆品",
        "水乳": "化妆品", "乳液": "化妆品",
        "护肤品": "化妆品", "美妆": "化妆品", "彩妆": "化妆品", "护肤": "化妆品",
        # ── 服饰运动 → 服饰（子品类 + 别名） ──
        "T恤": "服饰", "T恤衫": "服饰",
        "跑鞋": "服饰", "运动鞋": "服饰", "徒步鞋": "服饰", "登山鞋": "服饰",
        "篮球鞋": "服饰", "帆布鞋": "服饰", "板鞋": "服饰",
        "背包": "服饰",
        "连衣裙": "服饰", "牛仔裤": "服饰", "衬衫": "服饰",
        "卫衣": "服饰", "外套": "服饰", "短裤": "服饰", "瑜伽裤": "服饰",
        "拖鞋": "服饰", "帽子": "服饰", "墨镜": "服饰",
        # ── 数码电子 → 数码（子品类 + 别名） ──
        "耳机": "数码", "蓝牙耳机": "数码",
        "手机": "数码",
        "笔记本电脑": "数码", "笔记本": "数码", "平板": "数码", "平板电脑": "数码",
        "手表": "数码", "音箱": "数码",
        "充电器": "数码", "充电宝": "数码", "数据线": "数码",
        "键盘": "数码", "鼠标": "数码",
        # ── 食品饮料 → 食品（子品类 + 别名） ──
        "方便面": "食品", "牛肉面": "食品", "泡面": "食品",
        "零食": "食品", "饮料": "食品", "牛奶": "食品",
    ...
    def build_enriched_query(self, sid: str, question: str) -> str:
        ...
        if state and state.category:
            switched = False
            state_parent = self.CATEGORY_ALIASES.get(state.category, state.category)
            for cat in self.CATEGORIES:
                if cat in question:
                    cat_parent = self.CATEGORY_ALIASES.get(cat, cat)
                    if cat_parent != state_parent:
                        switched = True
                        break
            if not switched:
                for alias, cat in self.CATEGORY_ALIASES.items():
                    if alias in question:
                        alias_parent = self.CATEGORY_ALIASES.get(cat, cat)
                        if alias_parent != state_parent:
                            switched = True
                            break
