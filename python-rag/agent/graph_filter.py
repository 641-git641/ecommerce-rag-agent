...
def apply_diversity_exclusion(query: str, memory_state, ecommerce_graph, graph_filter: Optional[dict]) -> Optional[dict]:
    """多样性追问：硬排除上一轮品牌 + 返回排除的品牌名品牌列表

    Returns:
        (更新后的 graph_filter, 被排除的品牌名)
    """
    diversity_kw = ("另外", "其他", "别的", "不一样", "不同",
                    "换一个", "换一款", "换别的", "还有吗", "还有什么", "还有别的",
                    "再来", "再推", "再推荐", "除了这个", "除了这些", "除了")

    if not any(kw in query for kw in diversity_kw):
        return graph_filter, ""
    ...
    print(f"[Agent] 多样性追问: 排除品牌 '{prev_brand}'")
    ...
    try:
        prev_rounds = memory_state._product_memory.get("_sid_placeholder", [])
    except Exception:
        prev_rounds = []
    ...
    if exclude_pids:
        ...
        if "$in" in pid_filter:
            ...
            print(f"[Agent] 多样性追问: 硬排除品牌 '{prev_brand}' 的商品IDs ({before}→{after})")
        elif "$nin" in pid_filter:
            pid_filter["$nin"] = list(set(pid_filter["$nin"] + list(exclude_pids)))
            print(f"[Agent] 多样性追问: 硬排除品牌 '{prev_brand}' 的商品 {len(exclude_pids)} 个")
        else:
            pid_filter["$nin"] = list(exclude_pids)
            print(f"[Agent] 多样性追问: 硬排除品牌 '{prev_brand}' 的商品 {len(exclude_pids)} 个")

    return graph_filter if graph_filter else None, prev_brand
